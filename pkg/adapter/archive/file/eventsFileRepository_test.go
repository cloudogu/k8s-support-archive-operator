...
const (
	testEventsCollectorDirName  = "Events"
	testEventWorkDirArchivePath = testWorkPath + "/" + testNamespace + "/" + testName + "/" + testEventsCollectorDirName
	testWorkEvents              = testEventWorkDirArchivePath + "/events.log"
)

func TestEventFileRepository_createEventLog(t *testing.T) {
	type fields struct {
		workPath   string
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				...
				filesystem: func(t *testing.T, _ closableRWFile) volumeFs {
					...
					fsMock.EXPECT().MkdirAll(testEventWorkDirArchivePath, os.FileMode(0755)).Return(assert.AnError)
		...
		},
		{
			...
			fields: fields{
				...
				filesystem: func(t *testing.T, _ closableRWFile) volumeFs {
					fsMock := newMockVolumeFs(t)
					fsMock.EXPECT().MkdirAll(testEventWorkDirArchivePath, os.FileMode(0755)).Return(nil)
					fsMock.EXPECT().OpenFile(testWorkEvents, os.O_APPEND|os.O_CREATE|os.O_WRONLY, os.FileMode(0666)).Return(nil, assert.AnError)
			...
			wantErr: func(t *testing.T, err error) {
				require.Error(t, err)
				assert.ErrorIs(t, err, assert.AnError)
				assert.ErrorContains(t, err, "failed to create event file")
			},
		},
		{
			...
			fields: fields{
				...
				filesystem: func(t *testing.T, fileMock closableRWFile) volumeFs {
					fsMock := newMockVolumeFs(t)
					fsMock.EXPECT().MkdirAll(testEventWorkDirArchivePath, os.FileMode(0755)).Return(nil)
					fsMock.EXPECT().OpenFile(testWorkEvents, os.O_APPEND|os.O_CREATE|os.O_WRONLY, os.FileMode(0666)).Return(fileMock, nil)
			...
			wantErr: func(t *testing.T, err error) {
				require.Error(t, err)
				assert.ErrorIs(t, err, assert.AnError)
				assert.ErrorContains(t, err, "failed to write header to event file")
			},
		},
		{
			...
			wantErr: func(t *testing.T, err error) {
				require.Error(t, err)
				assert.ErrorIs(t, err, assert.AnError)
				assert.ErrorContains(t, err, "failed to write data to event file")
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			l := &EventFileRepository{
				workPath:   tt.fields.workPath,
				filesystem: tt.fields.filesystem(t, fileMock),
				eventFiles: tt.fields.eventFiles(tt.args.id, fileMock),
			}
			tt.wantErr(t, l.createEventLog(tt.args.ctx, tt.args.id, tt.args.data))
		})
	}
}
...
func TestNewEventFileRepository(t *testing.T) {
	...
	assert.NotNil(t, repository.eventFiles)
}

func TestEventFileRepository_close(t *testing.T) {
	type fields struct {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: func(t assert.TestingT, err error, i ...interface{}) bool {
				return assert.ErrorContains(t, err, "failed to close event file")
			},
		},
		{
